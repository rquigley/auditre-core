...
export class Parser {
     ...
     */
    parse(value: string, address?: string): { result: string | number | boolean | null; error: FormulaErrorId | null };
    ...
    on(
        hook: "callCellValue",
        handler: (cellCoord: { row: Coordinate; label: string; column: Coordinate }, done: DoneCallback, cellAddress: string | undefined) => void,
    ): void;
    on(
        hook: "callRangeValue",
        handler: (
     ...
     */
    setFunction(name: string, handler: (params: unknown, cellAddress: string | undefined) => unknown): Parser;

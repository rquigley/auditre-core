...
import Emitter from './helper/tiny-emitter';
...
class Parser extends Emitter {
  constructor() {
    ...
    this.parser.yy = {
      ...
      callFunction: (name, params) =>
        this._callFunction(name, params, this.cellAddress),
      cellValue: (value) => this._callCellValue(value, this.cellAddress),
      rangeValue: (start, end) =>
        this._callRangeValue(start, end, this.cellAddress),
    };
   ...
   */
  parse(expression, cellAddress) {
    let result = null;
    let error = null;
    this.cellAddress = cellAddress || undefined;
   ...
   * @private
   */
  _callFunction(name, params = [], cellAddress) {
    const fn = this.getFunction(name);
    let value;

    if (fn) {
      value = fn(params, cellAddress);
    }
   ...
   * @private
   */
  _callCellValue(label, cellAddress) {
   ...
   * @private
   */
  _callRangeValue(startLabel, endLabel, cellAddress) {

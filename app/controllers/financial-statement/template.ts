...
export const getPolicySections = () => [
  ...
  generateSection({
    ...
    body: (data) => `
      ...
      On [insert issue date of contract, Month, Day, Year (i.e. [${data.fiscalYearEnd}])], the Company entered into [Type of Financing Document]  (the “[Type of Financing Document]”) with several lenders (the “Lenders”). The Lenders provided the Company an aggregate cash consideration of [aggregate amount raised, $XX,XXX], net of issuance costs of [provided by the company]. The [Type of Financing Document] accrue interest at [insert annual insert rate] per annum, which is accrued on the note balance. All unpaid interest and principal are due and payable upon maturity on [insert maturity date, Month, Day, Year (i.e. [${data.fiscalYearEnd}]] (“Maturity Date”). In addition, at the election of the holders of a majority of the outstanding principal amount of the [Type of Financing Document], the outstanding principal balance of the [Type of Financing Document] is convertible at any time after the Maturity Date into shares of the Company’s Series A preferred stock at a conversion price equal to [number] per share.
  ...
  generateSection({
    ...
    body: (data) => `
      ...
      As of [${data.fiscalYearEnd}], each of the following events was considered a “Deemed Liquidation Event” unless the holders of a majority of the outstanding shares of convertible preferred stock elect by written notice sent to the Company at least 10 days prior to the effective date of any such event:
